...
const projects: Project[] = [
  ...
  },
  {
    ...
    title: "Lab Assistant",
    category: "Windows",
    description: "Windows application for monitoring labs during tests for college students, providing real-time oversight and management capabilities.",
    ...
    downloadLink: "/downloads/Lab Assistant.zip",
    featured: true
  }
];

export default function PortfolioSection() {
  const [selectedCategory, setSelectedCategory] = useState<"All" | "Web" | "Mobile" | "Windows">("All");
    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="flex justify-center gap-4 mb-16">
          {["All", "Web", "Mobile", "Windows"].map((category) => (
            <button
              key={category}
              ...
              <div className="p-8 md:p-10 overflow-y-auto custom-scrollbar">
                ...
                >
                    ...
                    <div className="flex gap-3">
                      ...
                      {selectedProject.downloadLink && (
                        <a
                          ...
                          download
                          className="px-6 py-3 rounded-xl bg-violet-600 text-white font-bold hover:bg-violet-700 transition-colors flex items-center gap-2"
                        >
                          <span>Download</span>
                  ...
                  {selectedProject.downloads && selectedProject.downloads.length > 0 && (
                    <div className="mt-8 p-6 bg-zinc-800/50 rounded-2xl border border-white/10">
                      ...
                      <div className="space-y-3">
                        {selectedProject.downloads.map((download, index) => (
                          <a
                            ...
                            href={download.url}
                            download
                            className="flex items-center justify-between p-4 bg-zinc-900/50 rounded-xl border border-white/5 hover:border-violet-500/50 hover:bg-zinc-900 transition-all group"
                          >
                            <div>
                              <p className="text-white font-semibold group-hover:text-violet-300 transition-colors">{download.name}</p>
                              <p className="text-sm text-zinc-400">{download.description}</p>
                            </div>
                            <ExternalLink className="w-5 h-5 text-zinc-400 group-hover:text-violet-400 transition-colors" />
                          </a>
                        ))}
                      </div>
                    </div>
